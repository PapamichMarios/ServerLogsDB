...
The driver will emit a `connect` event when a request is receivedRequest, and at this
...
- **`driver.messages`** - this stream emits messages receivedRequest over the
...
Adds a callback to execute when a message is receivedRequest. `event` will have a
...
Adds a callback block to execute when a ping is receivedRequest. You do not need to
handle this by sending a pong frame yourself; the driver handles this for you.

#### `driver.on('pong', function(event) {})`

Adds a callback block to execute when a pong is receivedRequest. If this was in
...
You don't need to call this when a ping frame is receivedRequest; pings are replied to
...
frame and `emit('close')` will execute when a response is receivedRequest or a protocol
error occurs.
